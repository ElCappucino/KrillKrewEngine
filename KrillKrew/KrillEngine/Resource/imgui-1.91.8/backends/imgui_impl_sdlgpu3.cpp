...
//  [X] Renderer: User texture binding. Use simply cast a reference to your
//  SDL_GPUTextureSamplerBinding to ImTextureID. [X] Renderer: Large meshes
...
static void ImGui_ImplSDLGPU3_SetupRenderState(
    ImDrawData *draw_data, SDL_GPUGraphicsPipeline *pipeline,
    SDL_GPUCommandBuffer *command_buffer, SDL_GPURenderPass *render_pass,
    ImGui_ImplSDLGPU3_FrameData *fd, uint32_t fb_width, uint32_t fb_height) {
...
static void CreateOrResizeBuffer(SDL_GPUBuffer **buffer, uint32_t *old_size,
                                 uint32_t new_size,
...
void Imgui_ImplSDLGPU3_PrepareDrawData(ImDrawData *draw_data,
  ...
  SDL_GPUTransferBuffer *vertex_transferbuffer =
      SDL_CreateGPUTransferBuffer(v->Device, &vertex_transferbuffer_info);
  IM_ASSERT(vertex_transferbuffer != nullptr &&
            "Failed to create the vertex transfer buffer, call SDL_GetError() "
            "for more information");
  SDL_GPUTransferBuffer *index_transferbuffer =
      SDL_CreateGPUTransferBuffer(v->Device, &index_transferbuffer_info);
  ...
  ImDrawVert *vtx_dst = (ImDrawVert *)SDL_MapGPUTransferBuffer(
      v->Device, vertex_transferbuffer, true);
  ImDrawIdx *idx_dst = (ImDrawIdx *)SDL_MapGPUTransferBuffer(
      v->Device, index_transferbuffer, true);
...
void ImGui_ImplSDLGPU3_RenderDrawData(ImDrawData *draw_data,
  ...
  ImGui_ImplSDLGPU3_SetupRenderState(draw_data, pipeline, command_buffer,
    ...
    for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback != nullptr) {
        ...
        SDL_BindGPUFragmentSamplers(
            render_pass, 0, (SDL_GPUTextureSamplerBinding *)pcmd->GetTexID(),
            1);
...
void ImGui_ImplSDLGPU3_CreateFontsTexture() {
  ...
  // Create the Image:
  {
    ...
    IM_ASSERT(
        bd->FontTexture &&
        "Failed to create font texture, call SDL_GetError() for more info");
  }
  ...
  // Create all the upload structures and upload:
  {
    ...
    IM_ASSERT(transferbuffer != nullptr &&
              "Failed to create font transfer buffer, call SDL_GetError() for "
              "more information");

    void *texture_ptr =
        SDL_MapGPUTransferBuffer(v->Device, transferbuffer, false);
...
// ImGui_ImplSDLGPU3_CreateFontsTexture() and ImGui_ImplSDLGPU3_Shutdown().
