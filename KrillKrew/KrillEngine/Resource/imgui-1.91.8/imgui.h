...
Bullet(); // draw a small circle + keep the cursor on the same line. advance
          ...
          // that TreeNode() uses
IMGUI_API bool
TextLink(const char *label); // hyperlink text button, return true when clicked
IMGUI_API void
TextLinkOpenURL(const char *label,
                const char *url = NULL); // hyperlink text button, automatically
                                         // open file/url when clicked
...
IMGUI_API void
SetNavCursorVisible(bool visible); // alter visibility of keyboard/gamepad
                                   // cursor. by default: show when using an
...
IMGUI_API bool IsMouseReleasedWithDelay(
    ImGuiMouseButton button,
    float delay); // delayed mouse release (use very sparingly!). Generally used
                  // with 'delay >= io.MouseDoubleClickTime' + combined with a
                  // 'io.MouseClickedLastCount==1' test. This is a very rarely
                  // used UI idiom, but some apps use this: e.g. MS Explorer
...
enum ImGuiChildFlags_ {
  ...
  ImGuiChildFlags_NavFlattened =
      1 << 8, // [BETA] Share focus scope, allow keyboard/gamepad navigation to
              // cross over parent border to this child or between sibling child
              // windows.

// Obsolete names
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
  ImGuiChildFlags_Border = ImGuiChildFlags_Borders, // Renamed in 1.91.1 (August
                                                    // 2024) for consistency.
#endif
};
...
enum ImGuiInputTextFlags_ {
  ...
  ImGuiInputTextFlags_CallbackResize =
      1 << 22, // Callback on buffer capacity changes request (beyond 'buf_size'
  ...
  ImGuiInputTextFlags_CallbackEdit =
      1 << 23, // Callback on any edit. Note that InputText() already returns
               // true on edit + you can always use IsItemEdited(). The callback
               // is useful to manipulate the underlying buffer while focus is
               // active.
...
enum ImGuiTreeNodeFlags_ {
  ...
  ImGuiTreeNodeFlags_SpanAllColumns =
      1 << 14, // Frame will span all columns of its container table (label will
  ...
  ImGuiTreeNodeFlags_NavLeftJumpsBackHere =
      1 << 17, // (WIP) Nav: left direction may move to this TreeNode() from any
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
  ImGuiKey_COUNT = ImGuiKey_NamedKey_END, // Obsoleted in 1.91.5 because it was
                                          // extremely misleading (since named
                                          // keys don't start at 0 anymore)
  ImGuiMod_Shortcut =
...
enum ImGuiConfigFlags_ {
  ...
  ImGuiConfigFlags_NavEnableSetMousePos =
      1
      << 2, // [moved/renamed in 1.91.4] -> use bool io.ConfigNavMoveSetMousePos
  ImGuiConfigFlags_NavNoCaptureKeyboard =
      1
      << 3, // [moved/renamed in 1.91.4] -> use bool io.ConfigNavCaptureKeyboard
#endif
};
...
enum ImGuiButtonFlags_ {
  ...
  ImGuiButtonFlags_EnableNav =
      1 << 3, // InvisibleButton(): do not disable navigation/tabbing. Otherwise
              // disabled by default.
};
...
enum ImGuiSliderFlags_ {
  ...
  ImGuiSliderFlags_NoSpeedTweaks =
      1 << 11, // Disable keyboard modifiers altering tweak speed. Useful if you
...
struct ImGuiIO {
  ...
  bool
      FontAllowUserScaling; // = false          // [OBSOLETE] Allow user scaling
  ...
  bool ConfigNavSwapGamepadButtons; // = false          // Swap Activate<>Cancel
  ...
  bool ConfigNavCaptureKeyboard; // = true           // Sets
                                 ...
                                 // set.
  bool ConfigNavEscapeClearFocusItem; // = true           // Pressing Escape can
                                      // clear focused item + navigation
  ...
  bool ConfigNavCursorVisibleAlways; // = false          // Navigation cursor is
                                     // always visible.
  ...
  bool
      ConfigWindowsCopyContentsWithCtrlC; // = false      // [EXPERIMENTAL]
                                          // CTRL+C copy the contents of focused
                                          // window into the clipboard.
                                          // Experimental because: (1) has known
                                          // issues with nested Begin/End pairs
                                          // (2) text output quality varies (3)
                                          // text output is in submission order
                                          // rather than spatial order.
  bool ConfigScrollbarScrollByPage; // = true           // Enable scrolling page
  ...
  //    - the later tends to facilitate iterating on code while running).
  ...
  ImGuiKeyData
      KeysData[ImGuiKey_NamedKey_COUNT]; // Key state for all known keys. Use
  ...
  double MouseReleasedTime[5]; // Time of last released (rarely used! but useful
...
enum ImGuiMultiSelectFlags_ {
  ...
  ImGuiMultiSelectFlags_BoxSelect1d =
      1 << 6, // Enable box-selection with same width and same x pos items (e.g.
              // full row Selectable()). Box-selection works better with little
              // bit of spacing between items hit-box in order to be able to aim
              // at empty space.
...
enum ImGuiSelectionRequestType {
  ...
  ImGuiSelectionRequestType_SetRange, // Request app to select/unselect
                                      ...
                                      // (inclusive) based on value of Selected.
                                      // Only EndMultiSelect() request this, app
                                      // code can read after BeginMultiSelect()
...
struct ImGuiSelectionBasicStorage {
  ...
  IMGUI_API bool
  Contains(ImGuiID id) const; // Query if an item id is in selection.
...
struct ImGuiSelectionExternalStorage {
  ...
  IMGUI_API void
  ApplyRequests(ImGuiMultiSelectIO *ms_io); // Apply selection requests by using
...
struct ImDrawCmd {
  ...
  void *UserCallbackData; // 4-8  // Callback user data (when UserCallback !=
                          // NULL). If called AddCallback() with size == 0, this
                          // is a copy of the AddCallback() argument. If called
                          // AddCallback() with size > 0, this is pointing to a
...
struct ImFontConfig {
  ...
  ImVec2
      GlyphExtraSpacing; // 0, 0     // Extra spacing (in pixels) between glyphs
                         // when rendered: essentially add to glyph->AdvanceX.
...
struct ImFontAtlasCustomRect {
  ...
  unsigned int GlyphColored : 1; // Input  // For custom font glyphs only: glyph
                                 // is colored, removed tinting.
  float
...
struct ImFont {
  ...
  ImVector<float> IndexAdvanceX; // 12-16 // out // Sparse. Glyphs->AdvanceX in
                                 // a directly indexable way (cache-friendly for
                                 // CalcTextSize functions which only this info,
  ...
  // Conceptually ConfigData[] is the list of font sources merged to create this
  // font.
  ...
  ImWchar EllipsisChar; // 2-4   // out // Character used for ellipsis rendering
                        // ('...').
  ImWchar FallbackChar; // 2-4   // out // Character used if a glyph isn't found
                        // (U+FFFD, '?')
  ...
  int MetricsTotalSurface; // 4     // out // Total surface in pixels to get an
                           // idea of the font rasterization/texture cost (not
                           // exact, we approximate the cost of padding between
                           // glyphs)
