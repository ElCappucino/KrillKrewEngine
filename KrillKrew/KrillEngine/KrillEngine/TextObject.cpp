...
void TextObject::loadText(std::string text, SDL_Color textColor, int fontSize) {
  ...
  TTF_Font* font = TTF_OpenFont("../Resource/Font/Debrosee-ALPnL.ttf",
  fontSize);

  if (font)
  {
          std::cout << "Have font." << std::endl;
          SDL_Surface* surfaceMessage = TTF_RenderText_Blended(font,
  text.c_str(), textColor);
          //SDL_Surface* formattedSurface =
  SDL_ConvertSurfaceFormat(surfaceMessage, SDL_PIXELFORMAT_RGBA32, 0);
          glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, surfaceMessage->w,
  surfaceMessage->h, 0, GL_RGBA, GL_UNSIGNED_BYTE, surfaceMessage->pixels);
  ...
  if (font) {
    ...
    SDL_Surface *formattedSurface =
        SDL_ConvertSurfaceFormat(surfaceMessage, SDL_PIXELFORMAT_RGBA32, 0);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, formattedSurface->w,
                 formattedSurface->h, 0, GL_RGBA, GL_UNSIGNED_BYTE,
...
float TextObject::getOrderingLayer() const { return -2300.f; }