...
void PlayerObject::SetIsSlow(bool isSlow)
{
    ...
    if (isSlow)
    {
        SpritesheetInfo slowSprite =
            SpritesheetInfo("slowSprite", "../Resource/Texture/Ability/abi_spr_jellyfish_effect.png", 55, 79, 55, 79);
...
void PlayerObject::UpdateCurrentAnimation()
{
    ...
    else if (currAnimState == AnimationState::Smash_DiagDown || currAnimState == AnimationState::Smash_DiagUp ||
             currAnimState == AnimationState::Smash_Side || currAnimState == AnimationState::Smash_Up ||
             currAnimState == AnimationState::Smash_Down || currAnimState == AnimationState::PlaceItem)
    {
        if (this->spriteRenderer->GetColumn() == (this->spriteRenderer->GetSheetWidth() / this->spriteRenderer->GetSpriteWidth()) - 1)
        {
            ...
            else
            {
             ...
             ((currAnimState != AnimationState::Move_Side) || (currAnimState != AnimationState::Move_Back) ||
...
void PlayerObject::UpdateCollider()
{
    ...
    glm::vec3 attackPos = glm::vec3(
        this->getPos().x + (this->GetCurrentDirection().x * 128.f), this->getPos().y - (this->GetCurrentDirection().y * 128.f), 0);
    ...
    // KK_TRACE("{0} groundColX = {1}, groundColY = {2}", player->GetPlayerNumber(), groundColX[player->GetPlayerNumber()],
    // groundColY[player->GetPlayerNumber()]);
    glm::vec3 groundCheckSize = glm::vec3(64.f, // change to modifiable size
                                          64.f,
                                          0);

    glm::vec3 groundCheckPos = glm::vec3(this->getPos().x + this->GetGroundColliderObject()->GetColliderOffset().x,
