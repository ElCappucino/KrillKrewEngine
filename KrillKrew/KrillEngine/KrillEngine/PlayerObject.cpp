...
void PlayerObject::Render(glm::mat4 globalModelTransform) {
  SquareMeshVbo *squareMesh = dynamic_cast<SquareMeshVbo *>(
      GameEngine::GetInstance()->GetRenderer()->GetMesh(
          SquareMeshVbo::MESH_NAME));

  GLuint modelMatixId =
      GameEngine::GetInstance()->GetRenderer()->GetModelMatrixAttrId();
  GLuint renderModeId =
      GameEngine::GetInstance()->GetRenderer()->GetModeUniformId();
  ...
  squareMesh->ChangeTextureData(
      spriteRenderer->GetRow(), spriteRenderer->GetColumn(),
      spriteRenderer->GetSpriteWidth(), spriteRenderer->GetSpriteHeight(),
      spriteRenderer->GetSheetWidth(), spriteRenderer->GetSheetHeight());
  ...
  if (squareMesh != nullptr) {
    ...
    glUniformMatrix4fv(modelMatixId, 1, GL_FALSE,
...
void PlayerObject::SetVelocity(float axisX, float axisY, bool isPositiveX,
                               bool isPositiveY) {
...
bool PlayerObject::GetXIsPositive() { return xIsPositive; }

bool PlayerObject::GetYIsPositive() { return yIsPositive; }
void PlayerObject::SetIsShooting(bool isShooting) {
  this->isShooting = isShooting;
}
void PlayerObject::SetIsAiming(bool isAiming) { this->isAiming = isAiming; }
void PlayerObject::SetPlayerNumber(int num) { this->playerNumber = num; }
void PlayerObject::SetAbilityCooldown(PlayerObject::AbilityButton button,
                                      int duration) {
...
void PlayerObject::SetIsSlow(bool isSlow) {
  ...
  if (isSlow) {
    SpritesheetInfo slowSprite = SpritesheetInfo(
        "slowSprite",
        "../Resource/Texture/Ability/abi_spr_jellyfish_effect.png", 55, 79, 55,
        79);
...
void PlayerObject::SetIsDashing(bool isDashing) { this->isDashing = isDashing; }
...
void PlayerObject::SetPlayerUI(UiObject *ui) { this->playerUI = ui; }
...
void PlayerObject::ReduceAbilityCooldown(int button, float dt) {
  ...
  if (abilityCooldown[button] <= 0) {
    abilityCooldown[button] = 0;
  }
  // KK_TRACE("Reduce Cooldown Skill: {0} Cooldown {1}", button,
  // GetCooldown(button));
}
void PlayerObject::ReduceSlowDuration(float dt) {
  ...
  // KK_TRACE("Reduce slow duration: Player Number {0} Duration: {1}",
  // GetPlayerNumber(), slowDuration);
}
...
void PlayerObject::SetAnimationSprite(AnimationState state,
...
void PlayerObject::ChangeAnimationState(AnimationState anim) {
  if (currAnimState != anim) {
    currAnimState = anim;
    this->SetTextureWithID(animList.find(anim)->second,
                           animList.find(anim)->second.textureid);
...
void PlayerObject::UpdateCurrentAnimation() {
  ...
  // End Animation After Finish
    ...
    if (this->spriteRenderer->GetColumn() ==
        (this->spriteRenderer->GetSheetWidth() /
         this->spriteRenderer->GetSpriteWidth()) -
            1) {
      if ((abs(this->velocity.x) > 0.f || abs(this->velocity.y) > 0.f)) {
        bool isHorizontalish =
            abs(currDirection.x) > abs(currDirection.y) ? true : false;
  ...
  } else if (((abs(this->velocity.x) > 0.f || abs(this->velocity.y) > 0.f)) &&
    ...
    bool */
    /*bool isHorizontalish = abs(currDirection.x) > abs(currDirection.y) ? true
    : false; bool isHeadingUp = currDirection.y > 0.f ? true : false; if
    (isHorizontalish)
    {
            if (isHeadingUp)
            {
  ...
  } else if (abs(this->velocity.x) <= 0.1f && abs(this->velocity.y) <= 0.1f) {
    ChangeAnimationState(AnimationState::Idle);
  }
}
Collider *PlayerObject::GetCollider() const { return collider; }
...
bool PlayerObject::GetIsOnGround() const { return isOnGround; }
...
glm::vec3 PlayerObject::GetVelocity() const { return velocity; }
bool PlayerObject::GetIsAiming() const { return isAiming; }
bool PlayerObject::GetIsSlow() const { return isSlow; }
bool PlayerObject::GetIsDashing() const { return isDashing; }
bool PlayerObject::GetIsShooting() const { return isShooting; }
int PlayerObject::GetPlayerNumber() const { return playerNumber; }
float PlayerObject::GetCooldown(PlayerObject::AbilityButton button) const {
  return abilityCooldown[static_cast<int>(button)];
}
float PlayerObject::GetSlowDuration() const { return slowDuration; }
float PlayerObject::GetDashDuration() const { return dashDuration; }
glm::vec2 PlayerObject::GetCurrentDirection() const { return currDirection; }

PlayerObject::Ability
PlayerObject::GetAbilityByButton(AbilityButton button) const {
...
void PlayerObject::HitAimingTile() {
  ...
  for (int i = 0; i < aimingProp.size(); i++) {
    if (aimingProp[i]->GetIsActive() == false) {
      auto clearTile =
          std::find(aimingProp.begin(), aimingProp.end(), aimingProp[i]);
  ...
  if (!aimingProp.empty()) {
    ...
    for (int i = 0; i < aimingTile.size(); i++) {
      if (aimingTile[i]->GetIsActive() == false) {
        auto clearTile =
            std::find(aimingTile.begin(), aimingTile.end(), aimingTile[i]);
        aimingTile.erase(clearTile);
      } else {
...
void PlayerObject::SetKnockbackDuration(int time) { durationKnockback = time; }
...
bool PlayerObject::GetIsKnockback() { return isKnockback; }

float PlayerObject::GetDurationKnockback() { return durationKnockback; }

void PlayerObject::SetIsTNT(bool isTNT) { this->isTNT = isTNT; }

bool PlayerObject::GetIsTNT() { return isTNT; }

void PlayerObject::SetHoldingProjectile(int projectile) {
  holdingProjectile = projectile;
}

int PlayerObject::GetHoldingProjectile() { return holdingProjectile; }

void PlayerObject::SetIsStun(bool isStun) { this->isStun = isStun; }

bool PlayerObject::GetIsStun() { return isStun; }

void PlayerObject::SetStunDuraion(int time) { durationStun = time; }
...
float PlayerObject::GetStunDuration() const { return durationStun; }

bool PlayerObject::GetIsFell() const { return this->isFell; }
bool PlayerObject::GetMeleeCooldown() const { return meleeCooldown; }
void PlayerObject::SetMeleeCooldown(float duration) {
  meleeCooldown = duration;
}
void PlayerObject::CheckIfOnGround() {
  if (!isOnGround) {
    /*std::cout << "Player is not on ground. groundCheckCollider Pos = " <<
       this->GetGroundColliderObject()->getPos().x << ", "
            << this->GetGroundColliderObject()->getPos().y << ", "
            << this->GetGroundColliderObject()->getPos().z << std::endl;*/
    // KK_TRACE("Player is not on ground. groundCheckCollider Pos = {0}",
    // this->GetGroundColliderObject()->getPos());
    this->SetVelocity(0, 0, false, false);
    this->isFell = true;
...
void PlayerObject::ApplyKnockback(EntityObject *obj) {
  ...
  if ((projectile != nullptr && projectile->GetCanKnockback() &&
       projectile->GetOwner()->GetPlayerNumber() != playerNumber) ||
      (trap != nullptr && trap->GetCanKnockback() &&
       trap->GetPlayerNumber() !=
           playerNumber)) // check if it is projectile or trap
  {

    glm::vec3 knockbackDirection = obj->getPos() - this->getPos();
    float magnitude =
        sqrt(std::pow(knockbackDirection.x, 2) + pow(knockbackDirection.y, 2));
    ...
    // KK_CORE_WARN("knockback Direction abs = {0}, {1}", knockbackDirectionX,
    // knockbackDirectionY);
    this->SetVelocity(knockbackDirectionX, knockbackDirectionY,
...
void PlayerObject::UpdateAbilityCooldown(float dt) {
  ...
  // KK_CORE_TRACE("PlayerObject: projectileHoldDuration = {0}",
  // projectileHoldDuration);
}
...
void PlayerObject::UpdateCollider() {
  ...
  glm::vec3 attackSize =
      glm::vec3(this->getSize().x / 4, this->getSize().y / 4, 0);
  glm::vec3 attackPos =
      glm::vec3(this->getPos().x + (this->GetCurrentDirection().x * 128.f),
                this->getPos().y - (this->GetCurrentDirection().y * 128.f), 0);
  ...
  // KK_TRACE("{0} groundColX = {1}, groundColY = {2}",
  // player->GetPlayerNumber(), groundColX[player->GetPlayerNumber()],
  // groundColY[player->GetPlayerNumber()]);
  glm::vec3 groundCheckSize = glm::vec3(64.f, // change to modifiable size
                                        64.f, 0);
  ...
  this->GetGroundColliderObject()->SetSize(groundCheckSize.x,
                                           groundCheckSize.y);
  ...
  if (this->slowStatusObject) {
    this->slowStatusObject->SetPosition(
        glm::vec3(this->pos.x + 48.f, this->pos.y + 48.f, 0));
  }
}
