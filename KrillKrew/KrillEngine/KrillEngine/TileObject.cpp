...
int BFSTile(std::array<std::array<int, MAP_WIDTH>, MAP_HEIGHT> &currentTile,
            int startX, int startY, int newFlag) {
  int tileAmount = 0;
  ...
  while (!checkQueue.empty()) {
    ...
    checkQueue.pop();
    if (current.x < 1 || current.x >= MAP_HEIGHT || current.y < 1 ||
        current.y >= MAP_WIDTH)
      continue;
  ...
  for (int i = 1; i < MAP_HEIGHT - 1; i++) {
    for (int j = 1; j < MAP_WIDTH - 1; j++) {
      ...
      if (bufferTile[i][j] != 1) // if not 1 skip
      {
        ...
        // if 1 bfs and replace it with replace value. updaate map[replace
        // value] then replace value++
...
TileObject::TileObject() {
  ...
  SpritesheetInfo crackEffectSprite = SpritesheetInfo(
      "CrackEffect", "../Resource/Texture/tileCrack.png", 128, 128, 512, 128);
...
void TileObject::SetSpriteInfo(SpritesheetInfo info) {
  this->spriteRenderer->SetSpriteInfo(info.spritewidth, info.spriteheight,
                                      info.sheetwidth, info.sheetheight);
...
void TileObject::SetIsBroke(bool isBroke) { this->isBroke = isBroke; }
bool TileObject::GetIsBreakable() const { return isBreakable; }
bool TileObject::GetIsBroke() const { return isBroke; }
...
Collider *TileObject::GetCollider() { return this->collider; }

ImageObject *TileObject::GetOverlaySprite() { return this->crackOverlay; }
...
void TileObject::SetUpdateTileset(
    std::array<std::array<int, MAP_WIDTH>, MAP_HEIGHT> *tiles) {
...
void TileObject::AddCollapseTileToScene() {
  ...
  SpritesheetInfo collapseTileSprite = SpritesheetInfo(
  ...
  particle->SetSize(particle->GetSpriteRenderer()->GetSpriteWidth(),
                    particle->GetSpriteRenderer()->GetSpriteHeight());
  ...
  collapseTile->GetSpriteRenderer()->ShiftTo(this->GetSpriteRenderer()->GetRow(),
  this->GetSpriteRenderer()->GetColumn());*/

  currentLevel->AddEntityToScene(particle);
}

float TileObject::getOrderingLayer() const { return 2000.f; }
