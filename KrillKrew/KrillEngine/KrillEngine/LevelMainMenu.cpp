...
LevelMainMenu::ButtonData* LevelMainMenu::InitButtonData(
...
void LevelMainMenu::InitializeMainMenuUI()
{
    ...
    // game name
    buttonList[MenuButtonName_GameName] = InitButtonData(MenuButtonName_GameName,
                                                         "MenuButtonName_GameName",
                                                         {100, 100},
                                                         {470, 55},
                                                         {0, 0},
                                                         0,
                                                         "../Resource/SceneData/MainMenu/MenuButtonName_GameName.json");
...
void LevelMainMenu::InitializeConfirmUI()
{
    ...
    auto SetupConfirmButton =
        [this, &ynsprite](MenuButtonName_ name, std::string stringName, glm::vec2 shift, std::vector<UiObject*>& targetList) {
    ...
    AreYouSure_No_ButtonPos.x =
        buttonList[MenuButtonName_AreYouSureStart_No]->pos.x + buttonList[MenuButtonName_AreYouSureStart_No]->offset.x;
...
void LevelMainMenu::InitializeOptionUI()
{
    ...
    MenuButtonName_ textButtons[] = {
        MenuButtonName_DisplayType_Text, MenuButtonName_MasterVolume_Text, MenuButtonName_SFXVolume_Text, MenuButtonName_BGMVolume_Text};

    std::string textButtonsName[] = {"MenuButtonName_DisplayType_Text",
    ...
    buttonList[MenuButtonName_DisplayType] =
        InitButtonData(MenuButtonName_DisplayType, "MenuButtonName_DisplayType", {100, 100}, {289, 65}, {0, 0}, 0, configFileName);
    ...
    UiObject* optionDisplay = InitUI(optionDisplaySprite,
                                     {displayTypePosX, displayTypePosY},
                                     {optionDisplaySprite.spritewidth, optionDisplaySprite.spriteheight},
                                     {0, 0});
    ...
    buttonList[MenuButtonName_DisplayDropdown] =
        InitButtonData(MenuButtonName_DisplayDropdown, "MenuButtonName_DisplayDropdown", {100, 100}, {289, 65}, {0, 0}, 0, configFileName);
    ...
    UiObject* displayDropdownUI = InitUI(displayDropdownSprite,
                                         {displayDropdownButtonPosX, displayDropdownButtonPosY},
                                         {displayDropdownSprite.spritewidth, displayDropdownSprite.spriteheight},
                                         {0, 0});
    ...
    std::string trackButtonsString[] = {
        "MenuButtonName_MasterVolume_Track", "MenuButtonName_SFXVolume_Track", "MenuButtonName_BGMVolume_Track"};
    std::string knobButtonsString[] = {
        "MenuButtonName_MasterVolume_Knob", "MenuButtonName_SFXVolume_Knob", "MenuButtonName_BGMVolume_Knob"};
    ...
    for (int j = 0; j < 3; j++)
    {
        ...
        UiObject* optionVolumeTrack =
            InitUI(trackSprite, {volumeButtonPosX, volumeButtonPosY}, {trackSprite.spritewidth, trackSprite.spriteheight}, {0, 0});
        ...
        UiObject* optionVolumeBox = InitUI(spriteList["OptionsVolumeBox"],
                                           {BoxButtonPosX, BoxButtonPosY},
                                           {spriteList["OptionsVolumeBox"].spritewidth, spriteList["OptionsVolumeBox"].spriteheight},
                                           {0, 0});
...
void LevelMainMenu::InitializeButtonsAction()
{
    ...
    buttonList[MenuButtonName_AreYouSureStart_No]->OnExecute = [this]() { BackToMainMenu(yesNoList_Start, MenuButtonName_StartButton); };

    buttonList[MenuButtonName_AreYouSureExit_Yes]->OnExecute = [this]() {
        gameEngine->GetStateController()->gameStateNext = GameState::GS_QUIT;
    };

    buttonList[MenuButtonName_AreYouSureExit_No]->OnExecute = [this]() { BackToMainMenu(yesNoList_Exit, MenuButtonName_ExitButton); };
                                     ...
                                     MenuButtonName_ defaultButtonKey)
{
    ...
    for (UiObject* ui : hideList)
        ui->SetIsRender(false);
    for (UiObject* ui : showList)
        ui->SetIsRender(true);
...
void LevelMainMenu::HandleOptionsCancel()
{
    ...
    if (name == MenuButtonName_DisplayType_Text || name == MenuButtonName_MasterVolume_Text || name == MenuButtonName_SFXVolume_Text ||
...
void LevelMainMenu::LevelInit()
{
    ...
    renderer->SetOrthoProjection(-(SCREEN_WIDTH / 2), (SCREEN_WIDTH / 2), -(SCREEN_HEIGHT / 2), (SCREEN_HEIGHT / 2));

    SDL_GetWindowSize(gameEngine->GetSDLWindow(), &windowWidth, &windowHeight);
    glViewport(0, 0, windowWidth, windowHeight);

    soundManager->LoadVolumeConfig("../Resource/SceneData/CurrentVolume.json",
                                   masterVolume,
                                   isToggleVolume[0],
                                   BGMVolume,
                                   isToggleVolume[2],
...
void LevelMainMenu::ShowImGuiConfig(bool isShowing)
{
    ...
    for (auto& pair : buttonList)
    {
        ...
        std::string displayName = buttonData->stringName.empty() ? "ID: " + std::to_string(enumKey) : buttonData->stringName;
    ...
    if (buttonList.find(selectedKey) != buttonList.end() && buttonList[selectedKey] != nullptr)
    {
        ...
        std::string inspectorTitle = currentData->stringName.empty() ? "Object Inspector" : currentData->stringName;
...
void LevelMainMenu::LoadAllButtonConfigs(const std::string& filename)
{
    ...
    for (auto& pair : buttonList)
    {
        ...
        if (rootJson.contains(key))
        {
            ...
            if (buttonJson.contains("posX"))
                buttonData->pos.x = buttonJson["posX"];
            if (buttonJson.contains("posY"))
                buttonData->pos.y = buttonJson["posY"];
            if (buttonJson.contains("width"))
                buttonData->size.x = buttonJson["width"];
            if (buttonJson.contains("height"))
                buttonData->size.y = buttonJson["height"];
            if (buttonJson.contains("offsetX"))
                buttonData->offset.x = buttonJson["offsetX"];
            if (buttonJson.contains("offsetY"))
                buttonData->offset.y = buttonJson["offsetY"];
            if (buttonJson.contains("column"))
                buttonData->column = buttonJson["column"];
...
TEST_CASE("LevelMainMenu Internal State Tests", "[InlineMenu]")
{
    ...
    else
    {
        ...
        window = SDL_CreateWindow(
            "My Game", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, WIDTH, HEIGHT, SDL_WINDOW_OPENGL | SDL_WINDOW_SHOWN);
        if (window == NULL)
        {
            // Display error message
