...
PlayerGroundColliderObject::PlayerGroundColliderObject(PlayerObject *parent) {
  ...
  glm::vec3 attackSize =
      glm::vec3(parent->getSize().x / 2, parent->getSize().y / 4, 0);
  glm::vec3 attackPos =
      glm::vec3(parent->getPos().x, parent->getPos().y - 128.f, 0);
  ...
  SpritesheetInfo hitboxSprite = SpritesheetInfo(
      "hitbox", "../Resource/Texture/marker01.png", 500, 500, 500, 500);

  this->SetSpriteInfo(hitboxSprite);

  this->orderingLayer = 2;
}
PlayerGroundColliderObject::~PlayerGroundColliderObject() { parent = nullptr; }
...
Collider *PlayerGroundColliderObject::GetCollider() { return this->collider; }
