...
public:
  ...
  /// @return It will return false if it can't initialize GLEW or OpenGL. return
  /// true if it's work normally.
  ...
  /// @brief The function for setup the clear color (color that paste over the
  /// screen during screen clearing process)
  ...
  /// @return It will return the model attribute if there is the model matrix in
  /// the shader data. return -1 if not.
  GLuint GetModelMatrixAttrId();
  /// @brief The getter function for Projection attribute of current shader.
  /// @return It will return the projection attribute if there is the projection
  /// matrix in the shader data. return -1 if not.
  GLuint GetProjectionMatrixAttrId();
  /// @brief The getter function for color uniform of current shader.
  /// @return It will return the color uniform attribute if there is the color
  /// uniform in the shader data. return -1 if not.
  GLuint GetColorUniformId();
  /// @brief The getter function for mode uniform of current shader.
  /// @return It will return the mode uniform attribute if there is the mode
  /// uniform in the shader data. return -1 if not.
