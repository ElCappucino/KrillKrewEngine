...
public:
    ...
    /// @return It will return the projection attribute if there is the projection matrix in the shader data. return -1
    /// if not.
    GLuint GetProjectionMatrixAttrId();
    /// @brief The getter function for color uniform of current shader.
    /// @return It will return the color uniform attribute if there is the color uniform in the shader data. return -1
    /// if not.
    GLuint GetColorUniformId();
    /// @brief The getter function for mode uniform of current shader.
    /// @return It will return the mode uniform attribute if there is the mode uniform in the shader data. return -1 if
    /// not.
