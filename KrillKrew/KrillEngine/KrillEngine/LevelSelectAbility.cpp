...
void LevelSelectAbility::LevelLoad()
{
    ...
    spriteList["Player_Window1"] =
        SpritesheetInfo("Player_Window1", "../Resource/Texture/CharacterSelection/UI_CS_PlayerWindow_1.png", 284, 373, 1988, 373);
    spriteList["Player_Window2"] =
        SpritesheetInfo("Player_Window2", "../Resource/Texture/CharacterSelection/UI_CS_PlayerWindow_2.png", 284, 373, 1988, 373);
    spriteList["Player_Window3"] =
        SpritesheetInfo("Player_Window3", "../Resource/Texture/CharacterSelection/UI_CS_PlayerWindow_3.png", 284, 373, 1988, 373);
    spriteList["Player_Window4"] =
        SpritesheetInfo("Player_Window4", "../Resource/Texture/CharacterSelection/UI_CS_PlayerWindow_4.png", 284, 373, 1988, 373);

    spriteList["Player_Icon"] =
        SpritesheetInfo("Player_Icon", "../Resource/Texture/CharacterSelection/UI_CS_PlayerProfile.png", 175, 152, 700, 152);
    spriteList["Player_Icon_Small"] =
        SpritesheetInfo("Player_Icon_Small", "../Resource/Texture/CharacterSelection/UI_CS_PlayerProfile_Small.png", 127, 120, 508, 120);

    spriteList["Ability_Icon"] =
        SpritesheetInfo("Ability_Icon", "../Resource/Texture/CharacterSelection/UI_CS_Abilityicon_Big.png", 160, 160, 1280, 160);
    spriteList["Ability_Icon_Small"] =
        SpritesheetInfo("Ability_Icon_Small", "../Resource/Texture/CharacterSelection/UI_CS_Abilityicon_Small.png", 72, 72, 648, 72);

    spriteList["Player_Remain"] =
        SpritesheetInfo("Player_Remain", "../Resource/Texture/CharacterSelection/UI_CS_PlayerRemain.png", 292, 133, 2044, 133);
    spriteList["Player_Hover"] =
        SpritesheetInfo("Player_Hover", "../Resource/Texture/CharacterSelection/PlayerAbilityHover.png", 450, 283, 1800, 283);
}
...
void LevelSelectAbility::LevelInit()
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        PlayerWindow->SetPosition(
            glm::vec3(configs.at(0)->posX + (configs.at(0)->offSetX * axisX), configs.at(0)->posY + (configs.at(0)->offSetY * axisY), 0));
        axisX++;
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        PlayerIcon->SetPosition(
            glm::vec3(configs.at(5)->posX + (configs.at(5)->offSetX * axisX), configs.at(5)->posY + (configs.at(5)->offSetY * axisY), 0));
        axisX++;
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        PlayerIconSmall->SetPosition(
            glm::vec3(configs.at(6)->posX + (configs.at(6)->offSetX * axisX), configs.at(6)->posY + (configs.at(6)->offSetY * axisY), 0));
        axisX++;
        ...
        for (int j = 0; j < 3; j++)
        {
            ...
            if (axisX < 3)
            {
                ...
                abilityIconSmall->SetPosition(glm::vec3(
                    configs.at(7)->posX + (configs.at(7)->offSetX * axisX), configs.at(7)->posY + (configs.at(7)->offSetY * axisY), 0));
            }
            else
            {
                abilityIconSmall->SetSize(configs.at(7)->width, -configs.at(7)->height);
                abilityIconSmall->SetPosition(
                    glm::vec3(configs.at(7)->posX + (configs.at(7)->offSetX * (axisX - 3)) + configs.at(0)->offSetX,
                              configs.at(7)->posY + (configs.at(7)->offSetY * axisY),
                              0));
            }
    ...
    if (SDL_NumJoysticks() > 0)
    {
        ...
        for (int i = 0; i < SDL_NumJoysticks(); i++)
        {
            ...
            if (playerWindowList.at(i)->GetSpriteRenderer()->GetColumn() != 0 && ready[i] == false && i < playerHoverList.size())
            {
                ...
                {
                    if (Joystick::GetButtonDown(i, Joystick::Button::DPAD_Right) || Joystick::GetButtonDown(i, Joystick::Button::DPAD_Up) ||
                        right || up)
                    {
                        playerHoverWhere[i]++;
                        helpHover[i]++;
                    ...
                    if (Joystick::GetButtonDown(i, Joystick::Button::DPAD_Left) ||
                        Joystick::GetButtonDown(i, Joystick::Button::DPAD_Down) || left || down)
                    {
                        playerHoverWhere[i]--;
                        helpHover[i]--;
                    ...
                    if (playerHoverWhere[i] > 2)
                    {
                        playerHoverList.at(i)->SetPosition(
                            glm::vec3(configs.at(4)->posX + (configs.at(4)->offSetX * (playerHoverWhere[i] - 4 + helpHover[i])),
                                      configs.at(4)->posY + AbilityIconListDown.at(0)->getPos().y + configs.at(4)->offSetY,
                                      0));
                    }
                    else
                    {
                        playerHoverList.at(i)->SetPosition(glm::vec3(
                            configs.at(4)->posX + (configs.at(4)->offSetX * (playerHoverWhere[i] + helpHover[i])), configs.at(4)->posY, 0));
                    }
                }
                ...
                if (Joystick::GetButtonDown(i, Joystick::Button::L1))
                {
                    playerIconSmallList.at(i)->GetSpriteRenderer()->ShiftTo(playerIconSmallList.at(i)->GetSpriteRenderer()->GetRow(),
                ...
                if (Joystick::GetButtonDown(i, Joystick::Button::Square))
                {
                    ...
                    /*for (int j = 0; j < playerIconSmallList.size(); j++)
                    {
                        for (int k = 0; k < SDL_NumJoysticks(); k++) {
                            if (playerIconSmallList.at(j)->getNumOwner() == i && playerIconSmallList.at(j)->GetSpriteRenderer()->GetColumn()
                    == character[k]) { canReady = false;
                            }
                        }
                    }*/
                    ...
                    if (canReady == true)
                    {
                        playerWindowList.at(i)->GetSpriteRenderer()->ShiftTo(playerWindowList.at(i)->GetSpriteRenderer()->GetRow(),
                                                                             playerIconSmallList.at(i)->GetSpriteRenderer()->GetColumn() +
                                                                                 2);
                        // playerRemainList.at(0)->GetSpriteRenderer()->ShiftTo(playerRemainList.at(0)->GetSpriteRenderer()->GetRow(),
                        // playerRemainList.at(0)->GetSpriteRenderer()->GetColumn() + 1);

                        for (int j = 0; j < 3; j++)
                        {
                            playerAbilityList[i]->at(j)->SetPosition(
                                glm::vec3(playerAbilityList[i]->at(j)->getPos().x, playerAbilityList[i]->at(j)->getPos().y - 25, 0));
                        }
...
void LevelSelectAbility::UpdateUi()
{
    ...
    for (int i = 0; i < playerHoverList.size(); i++)
    {
        ...
        for (int j = 0; j < playerHoverList.size(); j++)
        {
            ...
            {
                playerHoverList.at(j)->SetPosition(
                    glm::vec3(playerHoverList.at(j)->getPos().x, playerHoverList.at(j)->getPos().y + (configs.at(4)->offSetY * count), 0));
                count++;
                if (playerHoverList.at(i)->getPos() == playerHoverList.at(j)->getPos())
                {
                    playerHoverList.at(j)->SetPosition(glm::vec3(
                        playerHoverList.at(j)->getPos().x, playerHoverList.at(j)->getPos().y + (configs.at(4)->offSetY * count), 0));
                }
            }
        }
    }
    ...
    for (int i = 0; i < playerAbilityList.size(); i++)
    {
        for (int j = 0; j < 3; j++)
        {
            ...
            playerAbilityList[i]->at(j)->GetSpriteRenderer()->ShiftTo(playerAbilityList[i]->at(j)->GetSpriteRenderer()->GetRow(),
                                                                      playerAbility[i][j]);
        }
    }
    ...
    /*for (int i = 0; i < playerWindowList.size(); i++) {
        ...
        playerWindowList.at(i)->SetPosition(glm::vec3(configs.at(0)->posX + (configs.at(0)->offSetX * axisX), configs.at(0)->posY +
    (configs.at(0)->offSetY * axisY), 0)); playerWindowList.at(i)->SetSize(configs.at(0)->width, -configs.at(0)->height); axisX++;
    }*/
    ...
    // Player hover
    /*for (int i = 0; i < playerHoverList.size(); i++) {
        playerHoverList.at(i)->SetPosition(glm::vec3(configs.at(4)->posX + (configs.at(4)->offSetX * i), configs.at(4)->posY +
    (configs.at(4)->offSetY * i), 0)); playerHoverList.at(i)->SetSize(configs.at(4)->width, -configs.at(4)->height); axisX++;
    }*/
    ...
    for (int i = 0; i < playerIconList.size(); i++) {
        ...
        playerIconList.at(i)->SetPosition(glm::vec3(configs.at(5)->posX + (configs.at(5)->offSetX * axisX), configs.at(5)->posY +
    ...
    for (int i = 0; i < playerIconSmallList.size(); i++) {
        ...
        playerIconSmallList.at(i)->SetPosition(glm::vec3(configs.at(6)->posX + (configs.at(6)->offSetX * axisX), configs.at(6)->posY +
    (configs.at(6)->offSetY * axisY), 0)); axisX++; playerIconSmallList.at(i)->SetSize(configs.at(6)->width, -configs.at(6)->height);
    }*/
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 3; j++) {
            if (axisX < 3) {
                ...
                playerAbilityList[i]->at(j)->SetSize(configs.at(7)->width, -configs.at(7)->height);
                playerAbilityList[i]->at(j)->SetPosition(glm::vec3(configs.at(7)->posX + (configs.at(7)->offSetX * (axisX - 3)) +
    configs.at(0)->offSetX, configs.at(7)->posY + (configs.at(7)->offSetY * axisY), 0));
            }
            axisX++;
        }
...
void LevelSelectAbility::LevelDraw()
{
    ...
    //					axisY = 1;
    //				}
    //				playerWindowList.at(i)->SetPosition(glm::vec3(configs.at(configNum)->posX + (configs.at(configNum)->offSetX * axisX),
    //configs.at(configNum)->posY + (configs.at(configNum)->offSetY * axisY), 0));
    ...
    //			int axisX = 1;
    //			for (int i = 0; i < AbilityIconListTop.size(); i++) {
    //				AbilityIconListTop.at(i)->SetPosition(glm::vec3(configs.at(configNum)->posX + (configs.at(configNum)->offSetX * axisX),
    //configs.at(configNum)->posY, 0)); 				AbilityIconListTop.at(i)->SetSize(configs.at(configNum)->width, -configs.at(configNum)->height);
    //				axisX++;
    //			}
    ...
    //			int axisX = 1;
    //			for (int i = 0; i < AbilityIconListDown.size(); i++) {
    //				AbilityIconListDown.at(i)->SetPosition(glm::vec3(configs.at(configNum)->posX + (configs.at(configNum)->offSetX * axisX),
    //configs.at(configNum)->posY, 0)); 				AbilityIconListDown.at(i)->SetSize(configs.at(configNum)->width, -configs.at(configNum)->height);
    //				axisX++;
    //			}
