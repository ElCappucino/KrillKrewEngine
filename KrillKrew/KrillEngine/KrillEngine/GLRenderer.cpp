...
bool GLRenderer::InitGL(std::string vertexShaderFile,
...
bool GLRenderer::Initialize(std::string vertexShaderFile,
                            std::string fragmentShaderFile) {
  // Success flag
  bool success = true;
...
void GLRenderer::Render(std::vector<DrawableObject *> &objList) {
  ...
  if (pMatrixId != -1) {
    glUniformMatrix4fv(pMatrixId, 1, GL_FALSE,
...
OrthographicValue GLRenderer::GetOrthovalue() { return this->orthoValue; }

void GLRenderer::ClearMesh() {
  for (std::map<std::string, MeshVbo *>::iterator it = shapes.begin();
       it != shapes.end(); it++) {
    delete it->second;
  }
...
void GLRenderer::SetOrthoProjection(float left, float right, float bottom,
                                    float top) {
...
GLuint GLRenderer::GetModelMatrixAttrId() { return this->mMatrixId; }

GLuint GLRenderer::GetProjectionMatrixAttrId() { return this->pMatrixId; }

GLuint GLRenderer::GetColorUniformId() { return this->colorUniformId; }

GLuint GLRenderer::GetModeUniformId() { return this->renderModeUniformId; }
