...
void Camera::LerpCamera(int size) {
  ...
  float currentRatio =
      std::round((currentWidth / currentHeight) * 100.f) / 100.f;
  ...
  if (newWidth == previousWidth && newHeight == previousHeight) {
    ...
    if (newRatio > SCREEN_RATIO_X) {
      ...
      current_bottom =
          maxBottom - (((newWidth / SCREEN_RATIO_X) - newHeight) / 2.f);
      ...
      GameEngine::GetInstance()->SetDrawArea(current_left, current_right,
                                             current_bottom, current_top);
    } else if (newRatio < SCREEN_RATIO_X) {
      current_top = maxTop;
      current_bottom = maxBottom;
      current_left =
          maxLeft - (((newHeight * SCREEN_RATIO_X) - newWidth) / 2.f);
      current_right =
          maxRight + (((newHeight * SCREEN_RATIO_X) - newWidth) / 2.f);
...
float Camera::GetCenterX() {
  return GameEngine::GetInstance()->GetRenderer()->GetOrthovalue().left +
         ((GameEngine::GetInstance()->GetRenderer()->GetOrthovalue().right -
           GameEngine::GetInstance()->GetRenderer()->GetOrthovalue().left) /
          2);
}
float Camera::GetCenterY() {
  return GameEngine::GetInstance()->GetRenderer()->GetOrthovalue().bottom +
         ((GameEngine::GetInstance()->GetRenderer()->GetOrthovalue().top -
           GameEngine::GetInstance()->GetRenderer()->GetOrthovalue().bottom) /
          2);
}
...
glm::vec3 Camera::ScreenToCameraPosition(glm::vec3 screenPos) {
  float posX =
      ((cameraWidth / (float)GameEngine::GetInstance()->GetWindowWidth()) *
       screenPos.x) -
      (cameraWidth / 2.f);
  float posY = -1 * ((cameraHeight /
                      (float)GameEngine::GetInstance()->GetWindowHeight()) *
                     screenPos.y) -
               (cameraHeight / 2.f);
