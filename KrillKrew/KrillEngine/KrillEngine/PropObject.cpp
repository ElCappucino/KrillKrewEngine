  ...
  if (currentDurability >= maxDurability) {
    ...
    if (propType == PropType::Tree) {
      ...
      for (int i = 0; i < particleAmount; i++) {
        ...
        glm::vec3 position =
            glm::vec3(this->pos.x,
                      (this->pos.y + (this->size.y / 2.f)) +
                          ((i * (-this->size.y) / (float)particleAmount)),
                      0.f);

        particleList[i]->SetPosition(position);
        particleList[i]->SetSize(
            particleList[i]->GetSpriteRenderer()->GetSpriteWidth() * 2.f,
            particleList[i]->GetSpriteRenderer()->GetSpriteHeight() * 2.f);
...
bool PropObject::GetIsBreakable() const { return this->isBreakable; }
Collider *PropObject::GetCollider() { return this->collider; }

void PropObject::UpdateCollider() {
  if (propType == PropType::Tree) {
    glm::vec3 size =
        glm::vec3(this->getSize().x / 4.f, this->getSize().x / 4.f, 0);
...
float PropObject::getOrderingLayer() const {
  ...
  //{
  //	//KK_CORE_INFO("pos.y = {0} Size y = {1} order = {2}", this->pos.y,
  //this->size.y, this->pos.y + this->size.y); 	return this->pos.y +
  //(this->size.y / 2.f);
  // }
}