...
void LevelWinRound::LevelLoad()
{
    ...
    spriteList["WinRound_RoundNum"] =
        SpritesheetInfo("WinRound_RoundNum", "../Resource/Texture/GameWin/UI_RW_RoundNum.png", 330, 125, 990, 125);

    spriteList["WinRound_AllChar"] =
        SpritesheetInfo("WinRound_AllChar", "../Resource/Texture/GameWin/UI_RW_PlayerWinner.png", 919, 950, 3676, 950);

    spriteList["WinRound_Profile"] =
        SpritesheetInfo("WinRound_Profile", "../Resource/Texture/GameWin/UI_RW_PlayerProfile.png", 600, 150, 600, 150);
...
void LevelWinRound::UpdateUI()
{
    ...
    Background_UI->SetSize(Background_size.x, Background_size.y);

    RoundNum_UI->SetPosition(RoundNum_pos);
    RoundNum_UI->SetSize(RoundNum_size.x, RoundNum_size.y);

    RoundWinCharacter_UI->SetPosition(RoundWinCharacter_pos);
    RoundWinCharacter_UI->SetSize(RoundWinCharacter_size.x, RoundWinCharacter_size.y);

    PlayerProfile_UI->SetPosition(PlayerProfile_pos);
    PlayerProfile_UI->SetSize(PlayerProfile_size.x, PlayerProfile_size.y);

    for (int i = 0; i < 4; i++)
    {
        PlayerScore_UI[i]->SetPosition(PlayerScore_pos[i]);
        PlayerScore_UI[i]->SetSize(PlayerScore_size.x, PlayerScore_size.y);
    }
}
