        ...
        for (int j = 0; j < objectsList.size(); j++)
        {
            ...
            if (player2 != nullptr)
            {
                if (trap->GetPlayerNumber() != player2->GetPlayerNumber())
                {
                    ...
                    glm::vec2 delta = glm::vec2(abs(trap->getPos().x - player2->getPos().x), abs(trap->getPos().y - player2->getPos().y));
    ...
    for (int i = 0; i < objectsList.size(); i++)
    {
        ...
        for (int j = 0; j < objectsList.size(); j++)
        {
            ...
            if (entity2 != nullptr)
            {
                ...
                glm::vec2 delta = glm::vec2(abs(entity1->getPos().x - entity2->getPos().x), abs(entity1->getPos().y - entity2->getPos().y));
        ...
        for (int j = 0; j < objectsList.size(); j++)
        {
            ...
            if (player != nullptr)
            {
                if (projectile->GetOwner()->GetPlayerNumber() != player->GetPlayerNumber())
                {
                    ...
                    glm::vec2 delta =
                        glm::vec2(abs(projectile->getPos().x - player->getPos().x), abs(projectile->getPos().y - player->getPos().y));
...
void LevelGameplay::UpdateMovement()
{
    for (int i = 0; i < /*SDL_NumJoysticks() + playerNum*/ 4; i++)
    {
        ...
        if (players[i]->GetIsKnockback() == true && (abs(players[i]->GetVelocity().x) / 5 < 0.05) &&
...
void LevelGameplay::UpdateUI()
{
    for (int i = 0; i < objectsList.size(); i++)
    {
        ...
        if (ui != nullptr)
        {
            ...
            ui->SetPosition(
                glm::vec3(GameEngine::GetInstance()->GetRenderer()->GetOrthovalue().left +
                              (((300 * (ui->getNumOwner() + 1)) - (85 * ui->getNumOwner())) * camera.GetCameraWidth() / 1246),
                          GameEngine::GetInstance()->GetRenderer()->GetOrthovalue().bottom + (50 * camera.GetCameraHeight() / 720),
                          0));
                ...
                if (! players[numPlayer]->GetIsAiming())
                {
                    if (players[numPlayer]->GetIsShooting())
                    {
                        for (int i = 0; i < objectsList.size(); i++) // find teleport
                        {
                            ...
                            if (Teleport != NULL)
                            {
                                if (Teleport->GetType() == ProjectileObject::TypeProjectile::Teleport &&
