...
void LevelGameplay::LevelLoad() {
  ...
  spriteList["Bomb"] = SpritesheetInfo(
      "Bomb", "../Resource/Texture/Bomb_icon.png", 256, 256, 256, 256);
  spriteList["Shark_run"] =
      SpritesheetInfo("Shark_run", "../Resource/Texture/shark_run_test.png",
                      256, 256, 1024, 256);
  spriteList["Shark_idle"] =
      SpritesheetInfo("Shart_idle", "../Resource/Texture/shark_Idle_test.png",
                      256, 256, 256, 256);

  spriteList["Trap"] = SpritesheetInfo("Trap", "../Resource/Texture/Trap.png",
                                       512, 512, 512, 512);
  spriteList["TNT"] =
      SpritesheetInfo("TNT", "../Resource/Texture/TNT.png", 348, 348, 348, 348);
  spriteList["Teleport"] = SpritesheetInfo(
      "Teleport", "../Resource/Texture/EnderPearl.png", 512, 512, 512, 512);
  spriteList["Bola"] = SpritesheetInfo("Bola", "../Resource/Texture/Bola.png",
                                       317, 314, 317, 314);
  spriteList["Cleave"] = SpritesheetInfo(
      "Cleave", "../Resource/Texture/Cleave.png", 647, 386, 647, 386);
...
void LevelGameplay::LevelInit() {
  ...
  GameEngine::GetInstance()->GetRenderer()->SetClearColor(1.0f, 1.0f,
                                                          200.0f / 255);
  ...
  obj1->SetAnimationSprite(PlayerObject::AnimationState::Idle,
                           spriteList.find("Shark_idle")->second);
  // obj1->SetAnimationSprite(PlayerObject::AnimationState::Running,
  // spriteList.find("Shark_run")->second);
  //  New enum type
  obj1->SetAbility(PlayerObject::AbilityButton::Triangle,
                   PlayerObject::Ability::Fireball);
  obj1->SetAbility(PlayerObject::AbilityButton::Circle,
                   PlayerObject::Ability::Trap);
  obj1->SetAbility(PlayerObject::AbilityButton::Cross,
                   PlayerObject::Ability::Cleave);
  /*obj1->setAbility(0, 5);
  obj1->setAbility(1, 1);
  obj1->setAbility(2, 2);*/
  ...
  obj2->SetAnimationSprite(PlayerObject::AnimationState::Idle,
                           spriteList.find("Shark_idle")->second);
  // obj2->SetAnimationSprite(PlayerObject::AnimationState::Running,
  // spriteList.find("Shark_run")->second);
  obj2->SetAbility(PlayerObject::AbilityButton::Triangle,
                   PlayerObject::Ability::Fireball);
  obj2->SetAbility(PlayerObject::AbilityButton::Circle,
                   PlayerObject::Ability::TNT);
  obj2->SetAbility(PlayerObject::AbilityButton::Cross,
                   PlayerObject::Ability::Dash);
  ...
  obj3->SetAnimationSprite(PlayerObject::AnimationState::Idle,
                           spriteList.find("Shark_idle")->second);
  // obj3->SetAnimationSprite(PlayerObject::AnimationState::Running,
  // spriteList.find("Shark_run")->second);
  obj3->SetAbility(PlayerObject::AbilityButton::Triangle,
                   PlayerObject::Ability::Fireball);
  obj3->SetAbility(PlayerObject::AbilityButton::Circle,
                   PlayerObject::Ability::Trap);
  obj3->SetAbility(PlayerObject::AbilityButton::Cross,
                   PlayerObject::Ability::Dash);
  ...
  obj4->SetAnimationSprite(PlayerObject::AnimationState::Idle,
                           spriteList.find("Shark_idle")->second);
  // obj4->SetAnimationSprite(PlayerObject::AnimationState::Running,
  // spriteList.find("Shark_run")->second);
  obj4->SetAbility(PlayerObject::AbilityButton::Triangle,
                   PlayerObject::Ability::Fireball);
  obj4->SetAbility(PlayerObject::AbilityButton::Circle,
                   PlayerObject::Ability::Trap);
  obj4->SetAbility(PlayerObject::AbilityButton::Cross,
                   PlayerObject::Ability::Dash);
  ...
  if (SDL_NumJoysticks() > 0) {
    ...
    for (int i = 0; i < SDL_NumJoysticks(); i++) {
        ...
        if (players[i + playerNum]->GetIsStun() == false) {
          if (players[i + playerNum]->GetIsDashing() == false) {
            if (players[i + playerNum]->GetIsAiming() == false) {
              players[i + playerNum]->SetVelocity(abs(norAxisX), abs(norAxisY),
                                                  isPositiveX, isPositiveY);
            }
            ...
            players[i + playerNum]->SetVelocity(abs(norAxisXOld),
                                                abs(norAxisYOld),
                                                isPositiveXOld, isPositiveYOld);
          }
        }
      }
      ...
      if (players[i + playerNum]->GetIsAiming() &&
          players[i + playerNum]->GetHoldingProjectile() != 0) {
        for (int j = 0; j < objectsList.size(); j++) {
          ProjectileObject *projectile =
              dynamic_cast<ProjectileObject *>(objectsList[j]);
          if ((projectile != nullptr) &&
              (players[i + playerNum]->GetPlayerNumber() ==
               projectile->GetOwner()->GetPlayerNumber()) &&
              (projectile->GetIsShooting() == false)) {
            if (abs(norAxisX) > 0 || norAxisY > 0) {
              projectile->SetVelocity(abs(norAxisX), abs(norAxisY), isPositiveX,
                                      isPositiveY);
            }
            projectile->SetPosition(
                players[i + playerNum]->getPos() +
      ...
      if (Joystick::GetButtonDown(i, Joystick::Button::Cross)) {
        if (players[i + playerNum]->GetCooldown(
...
void LevelGameplay::UpdateCollision() {
    ...
    for (int j = 0; j < objectsList.size(); j++) {
      ...
      if (player2 != nullptr) {
        if (trap->GetPlayerNumber() != player2->GetPlayerNumber()) {
          ...
          glm::vec2 delta =
              glm::vec2(abs(trap->getPos().x - player2->getPos().x),
                        abs(trap->getPos().y - player2->getPos().y));

          float overlapX = (abs(col1.GetHalfSize().x)) +
                           (abs(col2.GetHalfSize().x)) - delta.x;
          float overlapY = (abs(col1.GetHalfSize().y)) +
                           (abs(col2.GetHalfSize().y)) - delta.y;

          if (overlapX > 0 && overlapY > 0) {
            std::cout << "Trap :" << trap->GetPlayerNumber() << " hit "
                      << "Player" << player2->GetPlayerNumber() << std::endl;
  ...
  for (int i = 0; i < objectsList.size(); i++) {
    ...
    if (entity1 == nullptr ||
        entity1->GetCollider()->GetCollisionType() == Collider::Static) {
      continue;
    }
    for (int j = 0; j < objectsList.size(); j++) {
      if (i == j) {
      ...
      if (entity2 != nullptr) {
        ...
        glm::vec2 delta =
            glm::vec2(abs(entity1->getPos().x - entity2->getPos().x),
                      abs(entity1->getPos().y - entity2->getPos().y));

        glm::vec2 previousDelta =
            glm::vec2(abs(col1.GetPreviousPos().x - col2.GetPreviousPos().x),
                      abs(col1.GetPreviousPos().y - col2.GetPreviousPos().y));

        float overlapX =
            (abs(col1.GetHalfSize().x)) + (abs(col2.GetHalfSize().x)) - delta.x;
        float overlapY =
            (abs(col1.GetHalfSize().y)) + (abs(col2.GetHalfSize().y)) - delta.y;
        ...
        if (overlapX > 0 && overlapY > 0) {
          ...
          if (col1.GetCollisionType() == Collider::Kinematic &&
            ...
            if (previousOverlapX > 0) {
              ...
              newPos.y =
                  entity1->getPos().y + (overlapY * (isTopSide ? 1 : -1));
            }
            if (previousOverlapY > 0) {
              bool isLeftSide =
                  (col1.GetPreviousPos().x - col2.GetPreviousPos().x) < 0
                      ...
                      : false;
              newPos.x =
                  entity1->getPos().x + (overlapX * (isLeftSide ? -1 : 1));
            }
  ...
  for (int i = 0; i < objectsList.size(); i++) {
    ProjectileObject *projectile =
        dynamic_cast<ProjectileObject *>(objectsList[i]);
    ...
    for (int j = 0; j < objectsList.size(); j++) {
      ...
      if (player != nullptr) {
        if (projectile->GetOwner()->GetPlayerNumber() !=
            player->GetPlayerNumber()) {
          Collider col1 = *projectile->GetCollider();
          Collider col2 = *player->GetCollider();

          glm::vec2 delta =
              glm::vec2(abs(projectile->getPos().x - player->getPos().x),
          ...
          if (overlapX > 0 && overlapY > 0) {
            if (projectile->GetType() ==
              ...
              players[projectile->GetOwner()->GetPlayerNumber()]->SetPosition(
                  projectile->getPos());
            }
            ...
            players[projectile->GetOwner()->GetPlayerNumber()]->SetIsShooting(
                false);
            UpdateKnockback(projectile, player);
            if (players[projectile->GetOwner()->GetPlayerNumber()]
                    ->GetHoldingProjectile() == projectile->GetType()) {
              players[projectile->GetOwner()->GetPlayerNumber()]->SetIsAiming(
                  false);
              players[projectile->GetOwner()->GetPlayerNumber()]
                  ->SetHoldingProjectile(0);
            }
            objectsList.erase(objectsList.begin() + i);
          }
        }
...
void LevelGameplay::UpdateMovement() {
  for (int i = 0; i < /*SDL_NumJoysticks() + playerNum*/ 4; i++) {
    ...
    if (time05s >= 0.1f && players[i]->GetIsKnockback() == true) {
      ...
      std::cout << "DurationKnockback | " << players[i +
      playerNum]->getDurationKnockback() << std::endl;*/
      ...
      players[i]->SetVelocity(knockbackVeloX / 1.1, knockbackVeloY / 1.1,
...
void LevelGameplay::UpdateKnockback(DrawableObject *obj1,
                                    DrawableObject *obj2) {
  ...
  if (player != NULL && projectile != NULL && projectile->GetCanKnockback()) {
    if (projectile->GetOwner()->GetPlayerNumber() !=
      ...
      player->SetVelocity(knockbackDirectionX, knockbackDirectionY,
                          knockbackDirectionXisPositive,
                          knockbackDirectionYisPositive);
    ...
    if (ui != nullptr) {
      ...
      ui->SetPosition(glm::vec3(
          GameEngine::GetInstance()->GetRenderer()->GetOrthovalue().left +
              (((300 * (ui->getNumOwner() + 1)) - (85 * ui->getNumOwner())) *
               camera.GetCameraWidth() / 1246),
          GameEngine::GetInstance()->GetRenderer()->GetOrthovalue().bottom +
              (50 * camera.GetCameraHeight() / 720),
          0));
      ui->SetSize(215.f * camera.GetCameraWidth() / 1246,
                  -100.f * camera.GetCameraHeight() / 720);
    }
  }
}
...
void LevelGameplay::Movement(float axisX, float axisY, bool isPositiveX,
                             bool isPositiveY) {}

void LevelGameplay::UsingAbility(int numPlayer,
    ...
    case PlayerObject::Ability::TNT:
      if (!players[numPlayer]->GetIsTNT()) {
        ...
        for (int i = 0; i < objectsList.size(); i++) // find tnt
        {
          ...
          if (realTnt != NULL) {
            if (realTnt->GetType() == TrapObject::TypeTrap::Tnt &&
                realTnt->GetPlayerNumber() == numPlayer) {
              realTnt->SetCanKnockback(true);
              realTnt->SetIsActive(true);
              realTnt->GetCollider()->Update(glm::vec3(500, 500, 0),
                                             realTnt->getPos());
    ...
    case PlayerObject::Ability::Teleport:
      if (!players[numPlayer]->GetIsAiming()) {
        if (players[numPlayer]->GetIsShooting()) {
          ...
          {
            ProjectileObject *Teleport =
                dynamic_cast<ProjectileObject *>(objectsList[i]);
            if (Teleport != NULL) {
              if (Teleport->GetType() ==
                      ProjectileObject::TypeProjectile::Teleport &&
                  Teleport->GetOwner()->GetPlayerNumber() == numPlayer) {
...
void LevelGameplay::AimFireball(int numPlayer,
                                PlayerObject::AbilityButton button) {
...
void LevelGameplay::ShootFireball(int numPlayer,
                                  PlayerObject::AbilityButton button) {
  players[numPlayer]->SetIsAiming(false);
  for (int j = 0; j < objectsList.size(); j++) {
    ProjectileObject *projectile =
        dynamic_cast<ProjectileObject *>(objectsList[j]);
    if (projectile != nullptr && projectile->GetOwner() == players[numPlayer] &&
        projectile->GetType() == ProjectileObject::TypeProjectile::Fireball) {
...
void LevelGameplay::AimTeleport(int numPlayer,
                                PlayerObject::AbilityButton button) {
  players[numPlayer]->SetVelocity(0, 0, false, false);
  players[numPlayer]->SetIsAiming(true);
  players[numPlayer]->SetHoldingProjectile(
...
void LevelGameplay::ShootTeleport(int numPlayer,
                                  PlayerObject::AbilityButton button) {
  if (players[numPlayer]->GetIsShooting() == false) {
    ...
    for (int j = 0; j < objectsList.size(); j++) {
      ProjectileObject *projectile =
          dynamic_cast<ProjectileObject *>(objectsList[j]);
      if (projectile != nullptr &&
          projectile->GetOwner() == players[numPlayer] &&
          projectile->GetType() == ProjectileObject::TypeProjectile::Teleport) {
        projectile->SetLifeTime(2);
        projectile->SetIsShooting(true);
        players[numPlayer]->SetHoldingProjectile(0);
      }
...
void LevelGameplay::ShootBola(int numPlayer,
                              PlayerObject::AbilityButton button) {
  players[numPlayer]->SetIsAiming(false);
  for (int j = 0; j < objectsList.size(); j++) {
    ProjectileObject *projectile =
        dynamic_cast<ProjectileObject *>(objectsList[j]);
    if (projectile != nullptr && projectile->GetOwner() == players[numPlayer] &&
        projectile->GetType() == ProjectileObject::TypeProjectile::Bola) {
      projectile->SetLifeTime(2);
      projectile->SetIsShooting(true);
      players[numPlayer]->SetHoldingProjectile(0);
    }
...
void LevelGameplay::AimCleave(int numPlayer,
                              PlayerObject::AbilityButton button) {
  players[numPlayer]->SetVelocity(0, 0, false, false);
  players[numPlayer]->SetIsAiming(true);
  players[numPlayer]->SetHoldingProjectile(
...
void LevelGameplay::ShootCleave(int numPlayer,
                                PlayerObject::AbilityButton button) {
  players[numPlayer]->SetIsAiming(false);
  for (int j = 0; j < objectsList.size(); j++) {
    ProjectileObject *projectile =
        dynamic_cast<ProjectileObject *>(objectsList[j]);
    if (projectile != nullptr && projectile->GetOwner() == players[numPlayer] &&
        projectile->GetType() == ProjectileObject::TypeProjectile::Cleave) {
      projectile->SetLifeTime(2);
      projectile->SetIsShooting(true);
      players[numPlayer]->SetHoldingProjectile(0);
    }
