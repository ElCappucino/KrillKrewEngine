...
void LevelMapTest::LevelInit()
{
    ...
    for (int i = 0; i < MAP_HEIGHT; i++)
    {
        for (int j = 0; j < MAP_WIDTH; j++)
        {
            ...
            if (i == 0 || i == MAP_HEIGHT - 1 || j == 0 || j == MAP_WIDTH - 1 || flag == 0)
            {
                continue;
...
{
    // check if the length of horizontal and vertical of projection + (target projection - current projection) * zoomInfo.t is not exceed
    // the minimum and maximum. if not, calculate the lerp of left right bottom top and update using SetDrawArea().
}
