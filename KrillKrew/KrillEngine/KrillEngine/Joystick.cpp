...
std::unordered_map<int, std::unique_ptr<Joystick::Controller>>
    Joystick::availableJoysticks;
...
void Joystick::OnJoystickConnected(SDL_ControllerDeviceEvent &e) {
  ...
  if (SDL_JoystickOpen(deviceIndex) != NULL) {
    ...
    if (c->gc) {
      ...
      KK_INFO("Joystick Connected: Mapindex: {0} DeviceIndex: {1}", mapIndex,
              deviceIndex);

      availableJoysticks[mapIndex] = std::move(c);
    } else {
      KK_ERROR("Error: Unable to Connect joystick with the Device index : {0}",
               deviceIndex);
    }
  }
}
...
bool Joystick::GetButtonDown(int joystickId, Button button) {
  ...
  if (it != availableJoysticks.end()) {
    return it->second->buttons[static_cast<int>(button)] &&
           !it->second->lastButtons[static_cast<int>(
               button)]; // check if it's not last button (not hold)
  }
