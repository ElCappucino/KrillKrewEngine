...
void Level::HandleKey(char key)
{
    ...
    switch (key)
    {
        ...
        case 'r':
            ...
            break;
        case 'e':
            GameEngine::GetInstance()->GetStateController()->gameStateNext = GameState::GS_LEVELGAMEPLAY;
            ;
            break;
        ...
        case 'i':
            ...
            GameEngine::GetInstance()->SetDrawArea(
                GameEngine::GetInstance()->GetRenderer()->GetOrthovalue().left + SCREEN_WIDTH * ZOOM_VELOCITY,
...
void Level::HandleMouse(int type, int x, int y)
{
    ...
    // float cameraWidth = GameEngine::GetInstance()->GetRenderer()->GetOrthovalue().right -
    // GameEngine::GetInstance()->GetRenderer()->GetOrthovalue().left; float cameraHeight =
    ...
    // player->SetPosition(glm::vec3(((cameraWidth / (float)GameEngine::GetInstance()->GetWindowWidth()) * realX) -
    // (cameraWidth / 2.f), -(((cameraHeight / (float)GameEngine::GetInstance()->GetWindowHeight()) * realY) -
    // (cameraHeight / 2.f)), 0));

    // std::cout << "Player position x : " << cameraWidth / (float)GameEngine::GetInstance()->GetWindowWidth() * realX
    // << 	"Player position y : " << cameraHeight / (float)GameEngine::GetInstance()->GetWindowHeight() * realY <<
    // std::endl;
}
