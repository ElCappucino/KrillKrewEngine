...
static const char *ImGui_ImplSDL3_GetClipboardText(ImGuiContext *) {
  ...
  bd->ClipboardTextData =
      sdl_clipboard_text ? SDL_strdup(sdl_clipboard_text) : nullptr;
...
static void ImGui_ImplSDL3_PlatformSetImeData(ImGuiContext *,
...
ImGui_ImplSDL3_GetViewportForWindowID(SDL_WindowID window_id) {
  ImGui_ImplSDL3_Data *bd = ImGui_ImplSDL3_GetBackendData();
...
bool ImGui_ImplSDL3_ProcessEvent(const SDL_Event *event) {
  ...
  case SDL_EVENT_KEY_UP: {
    ...
    //     event->key.key, SDL_GetKeyName(event->key.key), event->key.scancode,
    //     SDL_GetScancodeName(event->key.scancode), event->key.mod);
    ImGuiKey key =
        ImGui_ImplSDL3_KeyEventToImGuiKey(event->key.key, event->key.scancode);
    io.AddKeyEvent(key, (event->type == SDL_EVENT_KEY_DOWN));
    io.SetKeyEventNativeData(
        key, event->key.key, event->key.scancode,
        event->key.scancode); // To support legacy indexing (<1.87 user code).
