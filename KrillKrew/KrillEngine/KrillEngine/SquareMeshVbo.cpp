...
void SquareMeshVbo::LoadData() {
  ...
  GLfloat texData[] = {0.5f, 0.0f, 1.0f, 0.0f, 1.0f, 1.0f, 0.5f, 1.0f};
  ...
  glBufferData(GL_ARRAY_BUFFER, 2 * 4 * sizeof(GLfloat), texData,
               GL_STATIC_DRAW);
}

void SquareMeshVbo::ChangeTextureData(float row, float column,
...
std::string SquareMeshVbo::GetMeshName() { return MESH_NAME; }

void SquareMeshVbo::Render() {
  if (this->posAttribId != -1) {
    glBindBuffer(GL_ARRAY_BUFFER, this->posVboId);
    glVertexAttribPointer(this->posAttribId, 2, GL_FLOAT, GL_FALSE,
                          2 * sizeof(GLfloat), NULL);
  }
  if (this->texAttribId != -1) {
    glBindBuffer(GL_ARRAY_BUFFER, this->texVboId);
    glVertexAttribPointer(this->texAttribId, 2, GL_FLOAT, GL_FALSE,
                          2 * sizeof(GLfloat), NULL);
  }
