...
namespace glm {
namespace detail {
template <typename T, std::size_t size, bool aligned> struct storage {
  typedef struct type {
    uint8 data[size];
  } type;
};
...
#define GLM_ALIGNED_STORAGE_TYPE_STRUCT(x)                                     \
  template <typename T> struct storage<T, x, true> {                           \
...
template <> struct storage<float, 16, true> {
  typedef glm_vec4 type;
};

template <> struct storage<int, 16, true> {
  typedef glm_ivec4 type;
};

template <> struct storage<unsigned int, 16, true> {
...
template <> struct storage<double, 32, true> {
  typedef glm_dvec4 type;
};
#endif

#if (GLM_ARCH & GLM_ARCH_AVX2_BIT)
template <> struct storage<int64, 32, true> {
  typedef glm_i64vec4 type;
};

template <> struct storage<uint64, 32, true> {
