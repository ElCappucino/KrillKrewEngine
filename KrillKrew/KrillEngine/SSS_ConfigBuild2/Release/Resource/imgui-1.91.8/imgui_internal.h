...
static inline float ImLinearRemapClamp(float s0, float s1, float d0, float d1,
                                       float x) {
...
enum ImGuiItemFlagsPrivate_ {
  ...
  ImGuiItemFlags_Disabled =
      1 << 10, // false     // Disable interactions (DOES NOT affect visuals. DO
               // NOT mix direct use of this with BeginDisabled(). See
  ...
  ImGuiItemFlags_Default_ =
      ImGuiItemFlags_AutoClosePopups, // Please don't change, use PushItemFlag()
                                      // instead.
...
struct IMGUI_API ImGuiInputTextState {
  ...
  ImVector<char>
      CallbackTextBackup; // temporary storage for callback to support automatic
...
struct ImGuiBoxSelectState {
  ...
  ImRect
      BoxSelectRectPrev; // Selection rectangle in absolute coordinates (derived
                         // every frame from BoxSelectStartPosRel and MousePos)
...
struct IMGUI_API ImGuiMultiSelectTempData {
  ...
  ImVec2 ScopeRectMin;
  ImVec2 BackupCursorMaxPos;
  ImGuiSelectionUserData LastSubmittedItem; // Copy of last submitted item data,
                                            // used to merge output ranges.
  ImGuiID BoxSelectId;
...
struct ImGuiViewportP : public ImGuiViewport {
  ...
  ImVec2 WorkInsetMax; // "
  ImVec2 BuildWorkInsetMin; // Work Area inset accumulator for current frame, to
                            // become next frame's WorkInset
...
struct ImGuiContext {
  ...
  bool
      ActiveIdUsingAllKeyboardKeys; // Active widget will want to read all
                                    // keyboard keys inputs. (this is a shortcut
                                    // for not taking ownership of 100+ keys,
  ...
  ImGuiWindow *TooltipPreviousWindow; // Window of last tooltip submitted during
                                      // the frame
  ImVector<char>
...
SetItemKeyOwner(ImGuiKey key,
                ImGuiInputFlags flags); // Set key owner to last item if it is
                                        // hovered or active. Equivalent to 'if
...
IMGUI_API bool ArrowButtonEx(const char *str_id, ImGuiDir dir, ImVec2 size_arg,
                             ImGuiButtonFlags flags = 0);
IMGUI_API bool ImageButtonEx(ImGuiID id, ImTextureID user_texture_id,
