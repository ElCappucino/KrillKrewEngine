...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureID user_texture_id,
...
void ImGui::TextLinkOpenURL(const char *label, const char *url) {
  ...
  SetItemTooltip(LocalizeGetMsg(ImGuiLocKey_OpenLink_s),
                 url); // It is more reassuring for user to _always_ display URL
                       // when we same as label
...
bool ImGui::InputScalar(const char *label, ImGuiDataType data_type,
  ...
  IM_ASSERT(
      ...
      0); // Not supported by InputScalar(). Please open an issue if you this
          // would be useful to you. Otherwise use IsItemDeactivatedAfterEdit()!
...
static ImVec2 InputTextCalcTextSize(ImGuiContext *ctx, const char *text_begin,
  ...
  while (s < text_end) {
    ...
    const float char_width =
        ((int)c < font->IndexAdvanceX.Size ? font->IndexAdvanceX.Data[c]
...
static void STB_TEXTEDIT_LAYOUTROW(StbTexteditRow *r, ImGuiInputTextState *obj,
  ...
  const ImVec2 size =
      InputTextCalcTextSize(obj->Ctx, text + line_start_idx,
                            text + obj->TextLen, &text_remaining, NULL, true);
  r->x0 = 0.0f;
  r->x1 = size.x;
...
void ImGui::PushPasswordFont() {
  ...
  IM_ASSERT(out_font->Glyphs.Size == 0 && out_font->IndexAdvanceX.Size == 0 &&
...
bool ImGui::InputTextEx(const char *label, const char *hint, char *buf,
  ...
  IM_ASSERT(
      !((flags & ImGuiInputTextFlags_ElideLeft) &&
        (flags & ImGuiInputTextFlags_Multiline))); // Multiline will not work
                                                   // with left-trimming
  ...
  if (g.ActiveId == id) {
    ...
    // to specify multiple mods, or to all mods combinaison into individual
    // shortcuts.
  ...
  if (render_cursor || render_selection) {
...

    {
      ...
      // measuring distance
      cursor_offset.x = InputTextCalcTextSize(
                            &g, ImStrbol(cursor_ptr, text_begin), cursor_ptr)
                            .x;
      cursor_offset.y = cursor_line_no * g.FontSize;
      if (selmin_line_no >= 0) {
        select_start_offset.x =
            InputTextCalcTextSize(&g, ImStrbol(selmin_ptr, text_begin),
                                  selmin_ptr)
                .x;
    ...
    if (render_selection) {
      ...
      for (const char *p = text_selected_begin; p < text_selected_end;) {
        ...
        if (rect_pos.y < clip_rect.y) {
          ...
          ImVec2 rect_size =
              InputTextCalcTextSize(&g, p, text_selected_end, &p, NULL, true);
          if (rect_size.x <= 0.0f)
...
bool ImGui::Selectable(const char *label, bool selected,
  ...
  if (!is_visible)
    if (!is_multi_select || !g.BoxSelectState.UnclipMode ||
        !g.BoxSelectState.UnclipRect.Overlaps(
            bb)) // Extra layer of "no logic clip" for box-select support (would
                 // be more overhead to add to ItemAdd)
      return false;
  ...
  // SpanAllColumns.
  if (is_visible)
    RenderTextClipped(pos, ImVec2(window->WorkRect.Max.x, pos.y + size.y),
...
bool ImGui::BeginBoxSelect(const ImRect &scope_rect, ImGuiWindow *window,
  ...
  if (ms_flags & ImGuiMultiSelectFlags_ScopeWindow) // Box-select scrolling only
  ...
  if (ms_flags & ImGuiMultiSelectFlags_BoxSelect2d)
    if (bs->BoxSelectRectPrev.Min.x != bs->BoxSelectRectCurr.Min.x ||
      ...
      bs->UnclipMode = true;
      bs->UnclipRect =
          bs->BoxSelectRectPrev; // FIXME-OPT: UnclipRect x coordinates could be
                                 // intersection of Prev and Curr rect on X
                                 // axis.
...
static void ImGuiSelectionBasicStorage_BatchSetItemSelected(
  ...
  if (selected && !is_contained)
    storage->Data.push_back(ImGuiStoragePair(
        id, selection_order)); // Push unsorted at end of vector, will be sorted
