 ...
 *
 * Returns an InputHandle_t for the controller that can be used with Steam Input
...
SDL_GameControllerGetSteamHandle(SDL_GameController *gamecontroller);
 ...
 *  (fully pressed) when reported by SDL_GameControllerGetAxis(). Note that this
 * is not the same range that will be reported by the lower-level
 * SDL_GetJoystickAxis().
 */
typedef enum {
