...
typedef enum {
  ...
  SDL_WINDOW_MOUSE_CAPTURE =
      0x00004000, /**< window has mouse captured (unrelated to INPUT_GRABBED) */
  SDL_WINDOW_ALWAYS_ON_TOP =
      0x00008000, /**< window should always be above others */
  SDL_WINDOW_SKIP_TASKBAR =
      0x00010000, /**< window should not be added to the taskbar */
  SDL_WINDOW_UTILITY =
      0x00020000, /**< window should be treated as a utility window */
  SDL_WINDOW_TOOLTIP = 0x00040000, /**< window should be treated as a tooltip */
  SDL_WINDOW_POPUP_MENU =
      0x00080000, /**< window should be treated as a popup menu */
...
typedef enum {
  ...
  SDL_WINDOWEVENT_CLOSE, /**< The window manager requests that the window be
                            closed */
  SDL_WINDOWEVENT_TAKE_FOCUS, /**< Window is being offered a focus (should
                                 SetWindowInputFocus() on itself or a subwindow,
                                 or ignore) */
  SDL_WINDOWEVENT_HIT_TEST    /**< Window had a hit test that wasn't
                                 SDL_HITTEST_NORMAL. */
} SDL_WindowEventID;
 ...
 *
 *  \return 0 on success, or -1 if no DPI information is available or the index
 * is out of range.
 *
 *  \sa SDL_GetNumVideoDisplays()
 */
 ...
 * and refresh_rate default to the desktop mode if they are 0.  The modes are
 ...
 *  \sa SDL_GetWindowPosition()
 */
extern DECLSPEC void SDLCALL SDL_SetWindowPosition(SDL_Window *window, int x,
                                                   int y);
 ...
 *  \sa SDL_SetWindowPosition()
 */
extern DECLSPEC void SDLCALL SDL_GetWindowPosition(SDL_Window *window, int *x,
                                                   int *y);
...
/**
 *  \brief Get the size of a window's borders (decorations) around the client
 * area.
 *
 *  \param window The window to query.
...
/**
 *  \brief Sets the window as a modal for another window (TODO: reconsider this
...
/**
 *  \brief Provide a callback that decides if a window region has special
 * properties.
 *
 ...
 *  \param w        Pointer to variable for storing the width in pixels, may be
 * NULL
 *  \param h        Pointer to variable for storing the height in pixels, may be
 * NULL
 *
