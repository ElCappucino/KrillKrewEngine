...
//  2025-01-15: DirectX12: Texture upload use the command queue provided in
//  ImGui_ImplDX12_InitInfo instead of creating its own. 2024-12-09: DirectX12:
//  Let user specifies the DepthStencilView format by setting
//  ImGui_ImplDX12_InitInfo::DSVFormat. 2024-11-15: DirectX12: *BREAKING CHANGE*
//  Changed ImGui_ImplDX12_Init() signature to take a ImGui_ImplDX12_InitInfo
//  struct. Legacy ImGui_ImplDX12_Init() signature is still supported (will
//  obsolete). 2024-11-15: DirectX12: *BREAKING CHANGE* User is now required to
//  pass function pointers to allocate/free SRV Descriptors. We provide
//  convenience legacy fields to pass a single descriptor, matching the old API,
//  but upcoming features will want multiple. 2024-10-23: DirectX12: Unmap()
//  call specify written range. The range is informational and may be used by
//  debug tools. 2024-10-07: DirectX12: Changed default texture sampler to Clamp
...
// Functions
static void
ImGui_ImplDX12_SetupRenderState(ImDrawData *draw_data,
...
static void ImGui_ImplDX12_CreateFontsTexture() {
  ...
  ImGui_ImplDX12_Texture *font_tex = &bd->FontTexture;
  {
    ...
    UINT upload_pitch = (width * 4 + D3D12_TEXTURE_DATA_PITCH_ALIGNMENT - 1u) &
    ...
    bd->pd3dDevice->CreateShaderResourceView(pTexture, &srvDesc,
                                             font_tex->hFontSrvCpuDescHandle);
...
void ImGui_ImplDX12_InvalidateDeviceObjects() {
  ...
  bd->InitInfo.SrvDescriptorFreeFn(&bd->InitInfo,
                                   font_tex->hFontSrvCpuDescHandle,
                                   font_tex->hFontSrvGpuDescHandle);
  SafeRelease(font_tex->pTextureResource);
...
bool ImGui_ImplDX12_Init(ImGui_ImplDX12_InitInfo *init_info) {
  ...
  init_info->SrvDescriptorAllocFn(&bd->InitInfo,
                                  &bd->FontTexture.hFontSrvCpuDescHandle,
                                  &bd->FontTexture.hFontSrvGpuDescHandle);
